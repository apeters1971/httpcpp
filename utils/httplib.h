...
#define CPPHTTPLIB_COMPRESSION_BUFSIZ size_t(65536u)
#endif
...
public:
  ...
  Result Propfind(const std::string &path, Progress progress);
  Result Propfind(const std::string &path, const Headers &headers,
             Progress progress);
  Result Propfind(const std::string &path, ContentReceiver content_receiver);
  Result Propfind(const std::string &path, const Headers &headers,
             ContentReceiver content_receiver);
  Result Propfind(const std::string &path, ContentReceiver content_receiver,
             Progress progress);
  Result Propfind(const std::string &path, const Headers &headers,
             ContentReceiver content_receiver, Progress progress);
  Result Propfind(const std::string &path, ResponseHandler response_handler,
             ContentReceiver content_receiver);
  Result Propfind(const std::string &path, const Headers &headers,
             ResponseHandler response_handler,
             ContentReceiver content_receiver);
...
inline bool read_content_with_length(Stream &strm, uint64_t len,
  ...
  while (r < len) {
    ...
    if (n <= 0) {
      out(buf, static_cast<size_t>(0), r, len);
      return false; }
...
inline bool read_content_without_length(Stream &strm,
  ...
  for (;;) {
    ...
    if (n <= 0) {
      // call with size 0 to indicate end of stream
      if (!out(buf, static_cast<size_t>(0), r, 0)) { return false; }
      return true; }
...
inline Result ClientImpl::Propfind(const std::string &path) {
  return Propfind(path, Headers(), Progress());
}

inline Result ClientImpl::Propfind(const std::string &path, Progress progress) {
  return Propfind(path, Headers(), std::move(progress));
}

inline Result ClientImpl::Propfind(const std::string &path, const Headers &headers) {
  return Propfind(path, headers, Progress());
}
...
inline Result ClientImpl::Propfind(const std::string &path,
                              ContentReceiver content_receiver) {
  return Propfind(path, Headers(), nullptr, std::move(content_receiver), nullptr);
}

inline Result ClientImpl::Propfind(const std::string &path,
                              ContentReceiver content_receiver,
                              Progress progress) {
  return Propfind(path, Headers(), nullptr, std::move(content_receiver),
             std::move(progress));
}
...
inline Result ClientImpl::Propfind(const std::string &path,
                              ResponseHandler response_handler,
                              ContentReceiver content_receiver) {
  return Propfind(path, Headers(), std::move(response_handler),
             std::move(content_receiver), nullptr);
}
...
inline Result ClientImpl::Propfind(const std::string &path, const Params &params,
                              const Headers &headers, Progress progress) {
  if (params.empty()) { return Propfind(path, headers); }

  std::string path_with_query = append_query_params(path, params);
  return Propfind(path_with_query, headers, std::move(progress));
}
...
inline Result ClientImpl::Propfind(const std::string &path, const Params &params,
                              ...
                              ContentReceiver content_receiver,
                              Progress progress) {
  return Propfind(path, params, headers, nullptr, std::move(content_receiver),
             std::move(progress));
}
...
inline Result ClientImpl::Propfind(const std::string &path, const Params &params,
  ...
  std::string path_with_query = append_query_params(path, params);
  return Propfind(path_with_query, headers, std::move(response_handler),
             std::move(content_receiver), std::move(progress));
}
...
inline Result Client::Propfind(const std::string &path) { return cli_->Propfind(path); }
inline Result Client::Propfind(const std::string &path, const Headers &headers) {
  return cli_->Propfind(path, headers);
}
inline Result Client::Propfind(const std::string &path, Progress progress) {
  return cli_->Propfind(path, std::move(progress));
}
inline Result Client::Propfind(const std::string &path, const Headers &headers,
                          Progress progress) {
  return cli_->Propfind(path, headers, std::move(progress));
}
inline Result Client::Propfind(const std::string &path,
                          ContentReceiver content_receiver) {
  return cli_->Propfind(path, std::move(content_receiver));
}
inline Result Client::Propfind(const std::string &path, const Headers &headers,
                          ContentReceiver content_receiver) {
  return cli_->Propfind(path, headers, std::move(content_receiver));
}
inline Result Client::Propfind(const std::string &path,
                          ContentReceiver content_receiver, Progress progress) {
  return cli_->Propfind(path, std::move(content_receiver), std::move(progress));
}
inline Result Client::Propfind(const std::string &path, const Headers &headers,
                          ContentReceiver content_receiver, Progress progress) {
  return cli_->Propfind(path, headers, std::move(content_receiver),
...
inline Result Client::Propfind(const std::string &path,
                          ResponseHandler response_handler,
                          ContentReceiver content_receiver) {
  return cli_->Propfind(path, std::move(response_handler),
                   std::move(content_receiver));
}
inline Result Client::Propfind(const std::string &path, const Headers &headers,
                          ResponseHandler response_handler,
                          ContentReceiver content_receiver) {
  return cli_->Propfind(path, headers, std::move(response_handler),
                   std::move(content_receiver));
}
inline Result Client::Propfind(const std::string &path,
                          ResponseHandler response_handler,
...
inline Result Client::Propfind(const std::string &path, const Params &params,
                          const Headers &headers, Progress progress) {
  return cli_->Propfind(path, params, headers, std::move(progress));
}
inline Result Client::Propfind(const std::string &path, const Params &params,
                          const Headers &headers,
                          ContentReceiver content_receiver, Progress progress) {
  return cli_->Propfind(path, params, headers, std::move(content_receiver),
                   std::move(progress));
}
inline Result Client::Propfind(const std::string &path, const Params &params,
                          const Headers &headers,
                          ResponseHandler response_handler,
                          ContentReceiver content_receiver, Progress progress) {
  return cli_->Propfind(path, params, headers, std::move(response_handler),
                   std::move(content_receiver), std::move(progress));
}
